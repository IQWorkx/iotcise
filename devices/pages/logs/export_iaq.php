...
$sqlvv = "select * from device_parameter_config where device_id = '$device_id' and p_id = '4'";
...
$exp = mysqli_query($iot_db,"SELECT iaq,DATE_FORMAT(create_date, '%d-%M-%Y %H:%i') as create_date FROM `live_data` WHERE `device_id` = '$device_id' and create_date >= '$datefrom' and create_date <= '$dateto' order by create_date desc");
$header = "IAQ Value" . "\t" . "Date" . "\t";
...
header("Content-Disposition: attachment; filename=".$device_name."_iaq_data.xls");
