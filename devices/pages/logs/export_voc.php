...
$sqlvv = "select * from device_parameter_config where device_id = '$device_id' and p_id = '5'";
...
$exp = mysqli_query($iot_db,"SELECT cast(create_date AS date) as created_d,cast(create_date AS Time) as created_time,voc FROM `live_data` WHERE `device_id` = '$device_id' and date(create_date) >= '$datefrom' and date(create_date) <= '$dateto' order by create_date desc");
$header = "Date" . "\t" . "Time" . "\t" . "VOC Value" . "\t";
...
header("Content-Disposition: attachment; filename=".$device_name."_voc_data.xls");
