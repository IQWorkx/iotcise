...
$exp = mysqli_query($iot_db,"SELECT cast(create_date AS date) as created_d,cast(create_date AS Time) as created_time,pressure FROM `live_data` WHERE `device_id` = '$device_id' and date(create_date) >= '$datefrom' and date(create_date) <= '$dateto' order by create_date desc");
$header = "Date" . "\t" . "Time" . "\t" . "Pressure Value" . "\t";
