...
$exp = mysqli_query($iot_db,"SELECT cast(create_date AS date) as created_d,cast(create_date AS Time) as created_time,co2 FROM `live_data` WHERE `device_id` = '$device_id' and date(create_date) >= '$datefrom' and date(create_date) <= '$dateto' order by create_date desc");
$header = "Date" . "\t" . "Time" . "\t" . "CO2 Value" . "\t";
...
header("Content-Disposition: attachment; filename=".$device_name."_co2_data.xls");
